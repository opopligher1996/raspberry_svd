...
#                    needCapture = True
        ...
        for target in targets:
            tracker = target.getTracker()
            tracker.update(display_frame)
            pos = tracker.get_position()
            tracker_xmin = int(pos.left())
            tracker_ymin = int(pos.top())
            tracker_xmax = int(pos.right())
            tracker_ymax = int(pos.bottom())
            tracker_center_point = ( int((tracker_xmin + tracker_xmax)/2), int((tracker_ymin + tracker_ymax)/2))
            tracker_bbox = (tracker_xmin, tracker_ymin, tracker_xmax, tracker_ymax)
            min_iou = 0.5
            selected_tmp = None
            for t in tmp:
                t_center_point = t.getCenterPoint()
                (t_xmin,t_ymin,t_xmax,t_ymax) = t.getBBox()
                iou = cal_iou(tracker_bbox, t.getBBox())
                print(iou)
            cv2.rectangle(display_frame, (tracker_xmin, tracker_ymin), (tracker_xmax, tracker_ymax), (0, 0,0 ), 2)         
            cv2.circle(display_frame, tracker_center_point, 1, (0,0,0), 5)

        if(len(tmp) == len(targets)):
            print('len(tmp) == len(targets)')
