...
while True:
    ...
    if(tram == None and tmp != None):
        tram = tmp
        file_path = saveImage(frame1)
        tram.updateImagePath(file_path)
        if (isMatchedTram(tram) == True):
            print('Match the tram')
            matchedTram = tram
            sendData(matchedTram)
    elif(tram == None and tmp == None):
        tram == None
    elif(tram != None and tmp != None):
        tram.update(tmp)
    elif(tram != None and tmp == None):
    ...
    if(tram != None):
        ((xmin,ymin),(xmax,ymax)) = tram.getBBox()
        cv2.rectangle(frame1_resized, (xmin,ymin), (xmax,ymax), (10, 255, 0), 4)
        center_point = (int((xmin+xmax)/2), int((ymin+ymax)/2))
        cv2.circle(frame1_resized, center_point, 1, (10,255,0), 5)
        object_name = tram.getLabel()
        score = tram.getScore()
        label = '%s' % (object_name)
        labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
        label_ymin = max(ymin, labelSize[1] + 10)
        cv2.rectangle(frame1_resized, (xmin, label_ymin-labelSize[1]-10), (xmin+labelSize[0], label_ymin+baseLine-10),(255, 255, 255), cv2.FILLED)
        cv2.putText(frame1_resized, label, (xmin, label_ymin-7), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)                
